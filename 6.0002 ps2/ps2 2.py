...
an_edge7 = WeightedEdge("f", "e", 20, 7)
an_edge8 = WeightedEdge("e", "a", 90, 90)
an_edge9 = WeightedEdge("a", "g", 40, 0)
an_edge10 = WeightedEdge("g", "d", 20, 0)
...
agraph.add_edge(an_edge7)
agraph.add_edge(an_edge8)
agraph.add_edge(an_edge9)
agraph.add_edge(an_edge10)
...
def get_best_path(digraph, start, end, path, max_dist_outdoors, best_dist, best_path):
    ...
    if (not digraph.has_node(start)) or (not digraph.has_node(end)):
        raise ValueError("This node does no exist in this digraph")
    path = path + [start]
    if start == end:
        return path
    try:
        ...
        print("you are at a deadend node in graph ")
        return None
    for edge in start_edges: #edges associated with start node 
        node = edge.get_destination()
        if node not in path: #avoid cycles
            if (best_path == None) or (total_distance_of_path(digraph, path) < total_distance_of_path(digraph, best_path)): #starts recursive calling with latest node as start
                new_path = get_best_path(digraph, node, end, path, max_dist_outdoors, best_dist, best_path)
                if (new_path != None) and (outdoor_distance_of_path(digraph, new_path) <= max_dist_outdoors):
                    best_path = new_path
